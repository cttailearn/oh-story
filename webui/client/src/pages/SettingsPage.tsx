...
/** P6 设置页：渠道管理（增删/测试）+ 模型路由 + 预算 + 偏好（M1.8） */
    ...
    <div style={{ maxWidth: 960 }}>
            ...
            return (
              <div key={ch.id} style={{ border: '1px solid var(--line)', borderRadius: 6, padding: '12px', background: 'var(--paper)' }}>
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
                  <input
                    value={ch.name}
                    onChange={(e) => patchConfig((c) => { c.channels[i].name = e.target.value; return c; })}
                    style={inputStyle}
                    placeholder="渠道名"
                  />
                  <input
                    value={ch.base_url}
                    onChange={(e) => patchConfig((c) => { c.channels[i].base_url = e.target.value; return c; })}
                ...
                {ch.api_key && (
                  <div className="mono" style={{ fontSize: 12, color: 'var(--ink-2)', marginTop: 8 }}>
                    key: {ch.api_key}
                  </div>
                )}
                <div style={{ marginTop: 8 }}>
                  ...
                  <input
                    value={(ch.models ?? []).join(', ')}
                    onChange={(e) => patchConfig((c) => { c.channels[i].models = e.target.value.split(/[,，\s]+/).filter(Boolean); return c; })}
                    style={{ ...inputStyle, minWidth: 360, fontFamily: 'var(--font-mono)' }}
                    placeholder="gpt-image-2, deepseek-v4-pro, …"
                  />
                </div>
                {t?.ok && (
                  <div style={{ fontSize: 12.5, color: 'var(--green-jade)', marginTop: 6, lineHeight: 1.6 }}>
                    ✅ {t.msg} {t.models?.length ? `（发现 ${t.models.length} 个模型：${t.models.slice(0, 10).join('、')}…）` : ''}
                  </div>
                )}
                {t && t.ok === false && (
                  <div style={{ fontSize: 12.5, color: 'var(--red-vermillion)', marginTop: 6 }}>
                    ❌ {t.msg}（ping {t.llm?.ping_ms}ms）
                  </div>
                )}
              </div>
            );
      ...
      <div className="rail-block">
        ...
        {ROLE_KEYS.map((rk) => {
          ...
          const chosen = allModels.find((m: { channel: string; model: string }) => m.channel === cur?.channel && m.model === cur?.model);
          return (
            <div key={rk} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '7px 0', borderBottom: '1px dashed var(--line)' }}>
              ...
              <select
                ...
                onChange={(e) => {
                  ...
                  patchConfig((c) => {
                    ...
                    c.model_routing[rk] = { channel: channel as string, model: model as string };
                    return c;
                  });
                }}
                style={selectStyle}
              >
                ...
                {allModels.map((m: { channel: string; model: string }) => (
                  <option key={`${m.channel}/${m.model}`} value={`${m.channel}/${m.model}`}>
                    {m.channel} / {m.model}
                  </option>
                ))}
              </select>
              {chosen && <span className="mono" style={{ fontSize: 12, color: 'var(--green-jade)' }}>✓</span>}
            </div>
          );
        })}
        {allModels.length === 0 && (
      ...
      <div className="rail-block">
        ...
        <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13.5 }}>
          ...
          <select
            ...
            onChange={(e) => patchConfig((c) => ({ ...c, prefs: { ...c.prefs, theme: e.target.value } }))}
            style={selectStyle}
          >
        ...
        <div style={{ marginTop: 8 }}>
          <label style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 13.5 }}>
            <input
              ...
              onChange={(e) => patchConfig((c) => ({ ...c, prefs: { ...c.prefs, confirm_required: e.target.checked } }))}
            />
