...
const PY = process.env.PYTHON3 || (process.platform === "win32" ? "python" : "python3");
...
const runner = kind === "bash" ? BASH : kind === "py" ? PY : NODE;
  const label = `${i + 1}/${STEPS.length} [${kind}] ${script} ${args.join(" ")}`.trim();
  process.stdout.write(`${label} … `);
  const r = spawnSync(runner, [script, ...args], {
    cwd: repoRoot, encoding: "utf8", timeout: 600000, stdio: ["inherit", "pipe", "pipe"],
  });
  const head = (r.stdout || "").split("\n").slice(-2).map(s => s.trim()).filter(Boolean).join(" | ");
  if (r.status === 0) {
    console.log("OK" + (head ? ` — ${head}` : ""));
  } else {
    console.log(`FAIL (exit ${r.status})`);
    if (head) console.log("  " + head);
    const tailErr = (r.stderr || "").split("\n").slice(-3).map(s => s.trim()).filter(Boolean).join(" | ");
