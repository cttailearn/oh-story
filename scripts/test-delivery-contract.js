...
const verifier = path.join(
  repoRoot,
  "skills/story-short-write/scripts/check-delivery-contract.js",
);
...
try {
  ...
  assert.strictEqual(
    good.report.contract.metric,
    "non_whitespace_unicode_chars_v1",
  );
  ...
  const duplicateMarker = run(
    writeCase("duplicate-marker", body().replace("###6.", "###5.")),
  );
  assert.strictEqual(duplicateMarker.status, 1);
  assert.deepStrictEqual(failureIds(duplicateMarker), [
  ...
  assert.strictEqual(
    trailingBlank.status,
    0,
    trailingBlank.stdout + trailingBlank.stderr,
  );
