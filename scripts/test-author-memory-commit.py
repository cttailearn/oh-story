...
TOOL = REPO / "skills" / "story-setup" / "scripts" / "author_memory_commit.py"
...
def _loads(value: Any) -> Any:
    try:
        ...
        raise AssertionError(f"invalid JSON from tool output: {error}") from error


def transaction(
    transaction_id: str, revision: int, operations: list[dict[str, Any]]
) -> dict[str, Any]:
    return {
        "schema_version": 1,
...
def commit(
    workspace: Path, input_path: Path, document: dict[str, Any], *, expect: int = 0
...
def main() -> None:
    with tempfile.TemporaryDirectory(prefix="author-memory-test-") as temporary:
        ...
        empty_query = run(
            "query", "--workspace", str(workspace), "--kind", "prose_style"
        )
        assert _loads(empty_query.stdout)["items"] == []
        assert not memory.exists(), (
            "a read-only query must not initialize author memory"
        )

        init_result = run("init", "--workspace", str(workspace))
        assert _loads(init_result.stdout)["revision"] == 0
        ...
        assert _loads(replay.stdout)["replayed"]
        assert state(workspace)["state_revision"] == 1

        reused = _loads(json.dumps(first, ensure_ascii=False))
        ...
        partial_failure = transaction(
            ...
            [
                {
                    "action": "remember",
                    "preference": preference("不应落盘", "这条事务后面会失败。"),
                },
                {
                    "action": "forget",
                    "item_id": "AP999",
        ...
        profile.write_text(
            profile.read_text(encoding="utf-8") + "手工污染\n", encoding="utf-8"
        )
        ...
        recorded = _loads(record(workspace, input_path, record_event).stdout)
        ...
        query_document = _loads(queried.stdout)
        assert [item["id"] for item in query_document["items"]] == ["AP004", "AP005"]
        assert all(
            item["id"] not in {"AP001", "AP002", "AP003"}
            for item in query_document["items"]
        )
        ...
        forgotten = _loads(record(workspace, input_path, forget_event).stdout)
        assert forgotten["receipt"] == "Author Memory Receipt: r9 · AP005"
        replayed_record = _loads(record(workspace, input_path, record_event).stdout)
        assert replayed_record["replayed"]
        ...
        assert all(
            record["item_ids"] for record in final["applied_transactions"].values()
        )
        ...
        auto_event = {
            ...
            "operation": {
                "action": "remember",
                "preference": preference("偏好短标题", "记住：标题短一点。"),
            },
        }
        auto_result = _loads(record(auto_workspace, input_path, auto_event).stdout)
    ...
    for path, required_fragments in injection_contracts.items():
        ...
        for fragment in required_fragments:
            assert fragment in content, (
                f"missing author-memory injection contract in {path}: {fragment}"
            )
