...
test("用现有 demo 浏览拆文库、搜索项目并编辑保存", async ({
	page,
	request,
}) => {
	const consoleErrors = [];
	...
	await expect(page.locator("#projectsTab")).toHaveAttribute(
		"aria-selected",
		"true",
	);
	await page.locator("#projectsTab").press("ArrowLeft");
	await expect(page.locator("#librariesTab")).toHaveAttribute(
		"aria-selected",
		"true",
	);
	...
	await page
		.locator("#editorInput")
		.fill(`${await page.locator("#editorInput").inputValue()}${marker}`);
	...
	const activePath = await page
		.locator(".file-row[data-active='true']")
		.getAttribute("data-path");
	const persisted = await request.get(
		`/api/file?path=${encodeURIComponent(activePath)}`,
	);
	...
	await page
		.locator("#editorInput")
		.fill("<img src=x onerror=alert('unsafe')>\n\n# 安全预览");
	await page.getByRole("button", { name: "预览", exact: true }).click();
	await expect(page.locator("#previewPane")).toContainText(
	...
	await expect(page.locator("#fileTree")).toContainText("关系.md");
	expect(await page.locator("#fileTree .file-row").count()).toBe(
		unfilteredRows,
	);
...
test("从真实 demo 删除文稿前明确确认并刷新文件树", async ({
	page,
	request,
}, testInfo) => {
	const retryFiles = [
	...
	await expect(page.locator(`.file-row[data-path='${filePath}']`)).toHaveCount(
		1,
	);
	await expect(page.locator("#fileCount")).toHaveText(`${initialFileCount}+`);
	await expect(page.locator("#editorTitle")).toHaveText(fileName);
	const survived = await request.get(
	...
	await expect(page.locator("#fileCount")).toHaveText(
		`${initialFileCount - 1}+`,
	);
...
test("保存途中切走文稿，结果不能落到新打开的那份上", async ({
	page,
	request,
}) => {
	...
	await page
		.locator("#editorInput")
		.fill(`${await page.locator("#editorInput").inputValue()}${markerA}`);
	...
	await page
		.locator("#editorInput")
		.fill(`${await page.locator("#editorInput").inputValue()}${markerB}`);
	await expect(page.locator("#dirtyStatus")).toContainText("待保存");
	...
	const persistedA = await request.get(
		`/api/file?path=${encodeURIComponent(fileA)}`,
	);
	expect((await persistedA.json()).content).toContain(markerA.trim());
	const staleB = await request.get(
		`/api/file?path=${encodeURIComponent(fileB)}`,
	);
	...
	const savedB = await request.get(
		`/api/file?path=${encodeURIComponent(fileB)}`,
	);
...
test("保存途中继续敲的字仍算未保存", async ({ page, request }) => {
	...
	await page
		.locator("#editorInput")
		.fill(`${await page.locator("#editorInput").inputValue()}${saveMarker}`);
	...
	await page
		.locator("#editorInput")
		.fill(`${await page.locator("#editorInput").inputValue()}${lateMarker}`);
...
test("CRLF 稿件不会被一次改动重写换行，脏标记也能清干净", async ({
	page,
	request,
}) => {
	...
	const crlfContent = loaded.content
		.replaceAll("\r\n", "\n")
		.replaceAll("\n", "\r\n");
	...
	expect(saved.size).toBe(
		Buffer.byteLength(crlfContent) + Buffer.byteLength("改"),
	);
});

test("LF 稿件里的孤立 CR 不会把整篇保存成 CR-only", async ({
	page,
	request,
}) => {
	const filePath = "长篇/让你管账号，你高燃混剪炸全网/设定/文风.md";
...
test("CRLF 稿件里的孤立 CR 不会反向触发整篇 LF 重写", async ({
	page,
	request,
}) => {
	const filePath = "长篇/让你管账号，你高燃混剪炸全网/设定/文风.md";
...
test("深目录不再被首屏扫描深度剪掉，搜索时仍能按需找到", async ({
	page,
	request,
}) => {
...
test("宽目录按页加载，点击更多后继续追加而不丢失首批文件", async ({
	page,
	request,
}) => {
	const workspace = await request
			...
			await Promise.all(
				Array.from({ length: Math.min(100, 205 - start) }, (_, offset) =>
					writeFile(
						resolve(
							volume,
							`第${String(start + offset + 1).padStart(5, "0")}章.md`,
						),
						"凑数",
						"utf8",
					),
...
test("@mobile 手机视口仍可从真实长篇项目打开大纲", async ({ page }) => {
	...
	await page
		.locator(
			".file-row[data-path='长篇/让你管账号，你高燃混剪炸全网/大纲/大纲.md']",
		)
		.click();
