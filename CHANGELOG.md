...
> agents bundle 变化：7 个 agent 模板补「交付协议」硬约定（agents_version 27 → 28）。
...
### 拆解性能（story-long-analyze）

- **brief 章批处理**：一页摘要档从每章一个 agent 改为每批一个 agent 处理 8-10 章（64K 窗口）/15-20 章（128K），调用次数降 ~90%，200 章书 Stage 2 总成本降 ~72%
- **全量档默认批处理**：全量情节点章每批 1 个 agent 处理 3-5 章（64K）/8-10 章（128K），spawn 时 turnBudget 覆盖为 批章数×2+6；用户明确说「每章一个智能体/逐章处理」时才逐章。批内单章质量失败只单章重试（flash→pro 链），整批执行失败才整批重试

### 角色卡契约重构（character-basics / character-designer）

- **分时期特征**：身体/外貌特征改为分时期表（少年→青年、人类时期/细胞体时期），设定完善时在对应时期行下修改；分时期规则泛化到所有启用维度（能力进化、身份变迁、形态切换、道具换代）
- **特征维度库 + 分层密度**：12 维特征库（默认身体/外貌/语言/心理/特殊标记，按题材追加能力/形态/身份/等级境界/道具/生态/关系）；主角卡（全维度）/配角卡（功能相关 1-3 维简表）/龙套卡（一句话级）三层密度；反派密度随层级递进
- **用户自定义特征**：库外维度随时可加（如 身材特征: 身高/三围/体重/体型），声明格式 `自定义{维度名: 列1/列2/...}`，列头用户指定、首列固定时期/阶段
- **模板压缩**：身体+外貌合并为「外形特征」单表，语言/心理/特殊标记并入其它字段，其余维度需要哪个建哪个表
- **落盘机械校验**：角色卡落盘后主线程 grep 必查项 + 按启用维度校验（含硬事实表 key:value 单行禁表格——consistency-checker 第零步 grep 依赖），缺失回退补齐不静默

### 开书流程阶段门控（story-long-write）

- **Gate A（设定完善度）**：Phase 2→3 门控，核心设定表/角色卡/主要角色卡/关系/题材定位/提示卡/力量体系上限全达标才进大纲；不达标停靠询问，带缺写「设定待补」
- **角色线卡前移**：卷纲后、细纲前建核心角色角色线卡（原契约矛盾：artifact-protocols 说卷纲后、workflow-setup 说细纲后——细纲引用不存在的角色线）；与角色卡成长弧线/卷纲人物弧线三对齐
- **Gate B（大纲完成度）**：全部卷纲+核心角色角色线卡+大纲安全七检 才写细纲
- **Gate C（引用完整性）**：细纲落盘后 grep 校验「本章设定引用」点名的设定文件存在，缺则回补或改引用

### 大纲目录与角色线卡

- **大纲目录分层**：`大纲/大纲.md` 顶层鸟瞰 + `卷纲/`、`角色线/`、`细纲/` 三个子目录（去 卷纲_/细纲_ 前缀，角色线保留 角色线_ 前缀）；全库 21 文件路径引用、契约脚本、demo、老项目全部迁移；story-import 布局双兼容（导入老项目自动重建）
- **角色线卡去表格化**：阶段蓝图 7 列表格改为「阶段小节式」（每阶段一个 `## 阶段 N：{阶段名}（{卷/章范围}）` 小节，事件逐行列表可增补），细纲按阶段标题定位；线型支持自定义（主线|副线|感情线|成长线|事业线|调教线等自定义线名）
- **卷纲/细纲提示词强化**：写卷纲「人物弧线」前必读角色卡+角色线卡；写细纲前按「本章设定引用」加载涉及角色的角色卡+角色线卡（当前阶段行）

### 修复

- **turnBudget frontmatter 格式**（v1.3.1 内）：7 个 agent 模板 `turnBudget: { maxTurns: N }` → `{"maxTurns": N}` 合法 JSON，修复 pi-subagents 0.47.1 JSON.parse 崩溃导致 subagent 全挂
- **character-basics 共享副本漂移**：story-long-write/story-short-analyze 两份副本漏同步 87 行（特征维度库/龙套卡/外形特征合并），已用 sync-shared-assets 同步
