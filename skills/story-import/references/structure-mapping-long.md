...
> **布局双兼容**：读/写 `大纲/` 时先检测布局——新布局为子目录（`大纲/卷纲/第X卷.md`、`大纲/细纲/第N章.md`、`大纲/角色线/角色线_{名}.md`），旧布局为扁平文件（`大纲/卷纲_第X卷.md`、`大纲/细纲_第N章.md`、`大纲/角色线_{名}.md`）。导入老项目时：旧布局按新布局重建（文件移入子目录；卷纲/细纲去前缀，角色线保留 `角色线_` 前缀），不保留扁平写法；输出一律用新布局。
