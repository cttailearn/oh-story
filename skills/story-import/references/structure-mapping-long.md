...
> **布局双兼容**：读/写 `大纲/` 时先检测布局——新布局为子目录（`大纲/卷纲/第X卷.md`、`大纲/细纲/第N章.md`），旧布局为扁平文件（`大纲/卷纲_第X卷.md`、`大纲/细纲_第N章.md`）；角色剧情规划：**新格式在角色卡内「角色线」标题**（`设定/角色/{名}.md`），旧格式为独立文件（`大纲/角色线/角色线_{名}.md`，导入老项目时识别后映射进角色卡「角色线」标题，保留 `角色线_` 文件名信息在映射注释中）。导入老项目时：旧布局按新布局重建（文件移入子目录；卷纲/细纲去前缀）；输出一律用新布局。
