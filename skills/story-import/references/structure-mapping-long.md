...
> **布局双兼容**：读/写 `大纲/` 时先检测布局——新布局为子目录（`大纲/卷纲/第X卷.md`、`大纲/细纲/第N章.md`、`大纲/角色线/{线名}.md`），旧布局为扁平文件（`大纲/卷纲_第X卷.md`、`大纲/细纲_第N章.md`、`大纲/角色线/角色线_{名}.md`）。角色剧情规划格式：核心角色建独立线文件 `大纲/角色线/{线名}.md`（弧线定义 + 阶段规划 + 变化信号），并可在初始化 `arcs` 字段登记状态机骨架（见 tracking-transaction.md「角色线（弧线状态机）」）；功能型配角在角色卡「角色线」标题写一句功能定位。导入老项目时：旧独立文件 `大纲/角色线/角色线_{名}.md` 的内容**一次性重构**进新 `大纲/角色线/{名}.md` 或对应角色卡「角色线」标题；旧布局卷纲/细纲按新布局重建（文件移入子目录；卷纲/细纲去前缀）；输出一律用新布局。
...
3. **等待用户确认卷划分方案**后，才生成 `大纲/大纲.md` 的卷级结构和对应 `大纲/卷纲/第X卷.md`；
