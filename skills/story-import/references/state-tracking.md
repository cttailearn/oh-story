...
> 「角色线」（人物如何变化）的计划与进度分离：**计划**在 `大纲/角色线/{线名}.md`（独立线文件，由 character-designer 维护，模板见 character-basics.md 第 8 节）；**进度**由 `tracking_commit.py` 登记/推进，派生为 `追踪/角色线/{线名}.md`。本视图是派生文件，禁止手改。

- 只为核心角色建独立线文件；功能型配角不建，卡内一句功能定位即可。
- `追踪/角色线/{线名}.md` 列各阶段状态（已完成/进行中/计划）与证据锚点；`arc-audit` 输出每条线的 JSON 进度（status/achieved_stages/last_advance_chapter/overdue）。
- 写正文前，若细纲「本章设定引用 · 角色线:{名}·阶段N」点名了线，把 `追踪/角色线/{名}.md` 的当前阶段 + `大纲/角色线/{名}.md` 该阶段的「变化信号」纳入本节速记——本章要体现那个信号，否则阶段不推进（`delta.arc_advances` 只在验收信号达成时提交）。
- 状态快照（`角色状态/`）管「此刻是谁/在哪/想要什么」；进度视图（`角色线/`）管「这条线进行到哪一步」。两条不同轴，不要混写。
