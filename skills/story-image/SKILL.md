...
name: story-image
version: 2.1.0
description: "故事图像生成。生成小说封面、角色卡图（统一立绘/人设/三视图）、场景图，支持多后端（GPT-Image-2、GrsAI、火山方舟 Seedream、阿里通义万相、本地 ComfyUI），自动探测已配置后端，未配置时主动询问用户提供 API Key。触发方式：/skill:story-image、/story-image、/封面、/人物图、/立绘、/三视图、/角色卡图、/场景图、「帮我做个封面」「生成封面图」「做个小说封面」「封面设计」「生成人物形象图」「生成三视图」「生成场景图」「接入我的图像 API」「自定义 API」「我有 API 文档」「配置图像环境」；角色图未指定风格时默认国漫 3D 风格（斗罗大陆类 3D 动画渲染）。"
---
...
**风格选择优先级**（选定题材后定画风）：① 用户明确指定（"二次元/厚涂/水墨"等）→ ② 平台档（有目标平台时，见 visual-styles「平台风格」）→ ③ 题材风格库标签 → ④ **默认画风档**（兜底，未命中前几层一律用默认画风 = **国漫 3D 风格**，见 visual-styles「默认画风（兜底档）」）。角色图（char-sheet/portrait/turnaround）**未指定风格时默认画风档（国漫 3D）**，不随意选平台档。
...
**内置提示词模板（推荐）**：用 `scripts/prompt-template.py` 一键生成标准化提示词，避免手写拼接出错。 未指定风格时 **style 传 `general`（默认画风档 = 国漫 3D）或省略**（参数已可省略，缺省即 general），不要随意选平台档。
