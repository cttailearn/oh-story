...
story-image 支持五类图像生成（char-sheet 角色卡图为统一替代 portrait/turnaround 的新类型，旧两类兼容保留）。每类定义：触发意图、输入信息、提示词模板、输出目录、质量检查表。
...
| :----- | :----- | :--------- | :----------- | :--------- |
| `cover` 封面 | 封面、封面图 | `covers/<书名>/封面/` | 有（书名+作者名） | 番茄 1024x1536(3:4，`UPLOAD_SIZE` 裁 600x800)，默认 1024x1536(2:3) |
| **`char-sheet` 角色卡图（首选）** | 人物图、立绘、人设图、三视图、角色设定图 | `characters/<角色名>/` | 有（中文名+拼音+标语） | 1024x1536（竖版参考表） |
| `portrait` 人物形象图（兼容） | 人物图、立绘、人设图 | `characters/<角色名>/` | 无 | 1024x1536 |
| `turnaround` 三视图（兼容） | 三视图、正侧背面 | `characters/<角色名>/` | 无 | 1024x1024（三格） |
...
角色名（`设定/角色/{名}.md` 存在时用 `scripts/character-card.py` 自动提取素材）。选填：拼音、标语、表情网格、色板、简介、关键词标签、中英双语（--lang）。

### 提示词模板

见 [visual-styles.md](visual-styles.md)「角色卡图规范」；生成器：`scripts/prompt-template.py char-sheet`（模块化拼接，可按角色卡实际字段自动裁剪）。

### 一致性约束

同一角色的「外貌特征+服饰+发型」描述串固定复用，不逐次改写——角色卡图是后续封面/场景图的角色事实源。

### 质量检查

外貌与角色卡一致（基础信息/形象与能力逐项核对）、服饰符合身份题材、三视图同一角色（发型/服饰颜色/饰品位置/体态）、表情网格与角色性格吻合、无多余文字（除大标题/信息面板/标签外）。
...
| :----- | :-------- | :----- | :------------------- |
| 番茄小说 | 600×800 | 3:4 | `1024x1536` |
| 其它（默认竖版） | 按平台规格 | 2:3 | `1024x1536` |

> openai 后端兼容性：`gpt-image-2` 原生可传 `768x1024`（任意分辨率，16 的倍数且总像素 ≥655,360）；`gpt-image-1` 与兼容代理只接受 `1024x1024`/`1536x1024`/`1024x1536`/`auto`——**默认一律用 `1024x1536`**，番茄上传尺寸由 `UPLOAD_SIZE` 裁剪导出，不依赖后端任意分辨率能力。

有固定上传像素时设 `UPLOAD_SIZE`（番茄 `600x800`），生成后居中裁剪+缩放导出 `_上传` 版。裁剪工具按确定性顺序探测：`magick` → `convert` → `sips` → `python PIL`（`python -c "from PIL import Image"` 可用即用）；全部缺失时**显式报错并提示安装**（如 `winget install ImageMagick` / `brew install imagemagick`），不静默跳过。
