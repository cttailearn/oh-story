 ...
 * 分节标记与空行规则对齐 references/short-format.md（###1. / ###第N章 / 1.）。
 */
...
const MARKERS = [
  { style: "numeric-heading", pattern: /^###\s*\d+\.$/ },
  {
    ...
    pattern: /^###\s*第[一二三四五六七八九十百千万两〇零0-9]+章$/,
  },
...
function verify(projectDir, contract) {
  ...
  for (let index = 0; index < lines.length; index++) {
    ...
    const touchesMarker =
      markerLines.has(index - 1) || markerLines.has(index + 1);
  ...
  checks.push(
    makeCheck(
      ...
      invalidBlankLines.length
        ? `正文段落间空行：${invalidBlankLines.join("、")}`
        : "正文段落间无空行",
...
function parseArgs(argv) {
  ...
  if (!values["--min-chars"] || !values["--max-chars"] || !values["--sections"])
    return null;
...
function main(argv) {
  ...
  if (!parsed) {
    process.stderr.write(
      "用法: node scripts/check-delivery-contract.js --json --min-chars N --max-chars N --sections N [project-dir]\n",
    );
    return 2;
  }
