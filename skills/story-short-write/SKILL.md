...
7. 核心反转拆伏笔清单写入 `追踪/伏笔.md`（轻量追踪契约见 [references/light-tracking.md](references/light-tracking.md)，先有表后动笔）
...
- 每批写完后更新“已写小节摘要”（3-5 条：已揭示信息、情绪位置、未回收伏笔、下一批衔接句），并同步维护 `追踪/伏笔.md`（新埋补行、已收改状态；计划本批埋/收未做到的在小节大纲补注）
