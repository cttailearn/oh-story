...
const USAGE = "Usage: node check-revision-duplicate.js [--check] [--json] --original <file> --revised <file> [--mode rewrite|patch] [--exempt <file>]";
const MIN_LEN = 15;
...
function internalDuplicates(rev) {
  ...
  for (let i = 0; i + MIN_LEN <= rev.length; i++) {
    ...
    // 向后扩展
    let L = MIN_LEN;
    while (i + L < rev.length && j + L < rev.length && rev[i + L] === rev[j + L]) L++;
    // 向前扩展
    let i0 = i, j0 = j;
    while (i0 > 0 && j0 > 0 && rev[i0 - 1] === rev[j0 - 1]) { i0--; j0--; L++; }
...
function main() {
  ...
  let check = false, json = false, original = null, revised = null, mode = "rewrite", exemptFile = null;
  ...
  if (!original || !revised || !["rewrite", "patch"].includes(mode)) { console.error(USAGE); process.exit(2); }

  const origText = readText(original);
  const revText = readText(revised);
  if (origText === null || revText === null) {
    console.error("无法读取原稿或新稿"); process.exit(2);
  }
  ...
  if (mode === "rewrite") {
    ...
    for (const h of hits) {
      ...
      if (last && h - last.end < MIN_LEN) { last.end = h + MIN_LEN; last.hits.push(h); }
      else segments.push({ start: h, end: h + MIN_LEN, hits: [h] });
    }
    for (const seg of segments) {
      // 向两端扩展真实匹配长度
      let left = seg.start, right = seg.end;
      while (left > 0 && orig.includes(rev.slice(left - 1, right))) left--;
      while (right < rev.length && orig.includes(rev.slice(left, right + 1))) right++;
  ...
  for (const d of dups.slice(0, 10)) {
    issues.push({ type: "rev-dup", severity: "S1", snippet: d.slice(0, 40) + "…", message: "新稿内部重复：≥15 字片段出现 ≥2 次（引号内台词/弹幕/复沓锚句登记可豁免，其余需去重）" });
  }

  // 3. 字数统计
  const stats = { original: orig.length, revised: rev.length, delta: rev.length - orig.length };

  if (json) {
    console.log(JSON.stringify({ mode, stats, issues }, null, 2));
  } else {
    console.log(`--- 修订重复检测（mode=${mode}）---`);
    console.log(`  原稿 ${stats.original} 字 → 新稿 ${stats.revised} 字（${stats.delta >= 0 ? "+" : ""}${stats.delta}）`);
    if (issues.length === 0) console.log("  [PASS]");
    else for (const x of issues) console.log(`  [${x.severity}][${x.type}] ${x.message}：${x.snippet}`);
    console.log(issues.length ? `FAIL: ${issues.length} issue(s)` : "Result: PASS");
  }
