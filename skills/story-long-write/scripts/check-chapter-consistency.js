...
const USAGE =
  "Usage: node check-chapter-consistency.js [--check] [--json] --project <root>";
...
function main() {
  ...
  const chapters = globMd(project, "正文")
    .map((f) => ({ file: f, no: chapterNumber(f) }))
    .filter((x) => x.no !== null)
    .sort((a, b) => a.no - b.no);
  const outlines = globMd(project, "大纲/细纲")
    .map((f) => chapterNumber(f))
    .filter((x) => x !== null);
  ...
  for (const c of chapters) {
    if (!outlineSet.has(c.no))
      fails.push(
        `[S1][pair] 正文 ${c.file} 无对应细纲 大纲/细纲/第${c.no}章.md`,
      );
  }
  ...
  for (const c of chapters) {
    ...
    if (wm.length) {
      const last = wm[wm.length - 1][1];
      wd = WEEKDAY[last] || null;
    }
    if (wd !== null && lastWeekday !== 0) {
      // 同日多章（同星期）合法；倒退或跨周跳跃均提示人工核对
      if (wd < lastWeekday)
        hints.push(
          `[S2][timeline] 第${c.no}章 星期变化：前章周${lastWeekday}，本章周${wd}——倒退或跨周跳跃；若为跨周/倒叙/时间跳跃请忽略，否则人工核对`,
        );
    }
    ...
    if (dm.length) {
      ...
      if (lastMonthDay !== 0 && md < lastMonthDay)
        hints.push(
          `[S2][timeline] 第${c.no}章 日期变化：${last[0]} 早于前章 ${lastMonthDayText}——倒退或跨年；若为跨年/倒叙/时间跳跃请忽略，否则人工核对`,
        );
      if (md > lastMonthDay) {
        lastMonthDay = md;
        lastMonthDayText = last[0];
      }
    }
    ...
    if (dcm.length) {
      ...
      if (lastDayCount !== 0 && last < lastDayCount)
        hints.push(
          `[S2][timeline] 第${c.no}章 天数变化：第${last}天 早于前章 第${lastDayCount}天——倒退或跳段；若为时间跳跃/倒叙请忽略，否则人工核对`,
        );
    ...
    if (rm.length) {
      ...
      if (lastRemain !== Infinity && last > lastRemain)
        hints.push(
          `[S2][timeline] 第${c.no}章 倒计时变化：还剩${last}天 多于前章 还剩${lastRemain}天——回退或跨章重计；若为合法重计/倒叙请忽略，否则人工核对`,
        );
  ...
  if (json) {
    console.log(
      JSON.stringify(
        { checked: chapters.length, issues: fails, hints },
        null,
        2,
      ),
    );
    ...
    if (hints.length)
      console.log(
        `  （${hints.length} 条 S2 时间线提示：跨周/跨年/倒叙/时间跳跃属正常，请人工核对）`,
      );
    console.log(
      fails.length ? `FAIL: ${fails.length} issue(s)` : "Result: PASS",
    );
  }
