...
│   │   └── ...
│   ├── 角色/
│   │   ├── 沈栀.md            # 每个角色一份：基础信息/形象与能力/说话方式/角色线(线指针)/写作红线/本书专属
│   │   └── ...
│   ├── 势力/
│   │   ├── 天机阁.md          # 每个势力/组织一个文件
│   │   └── ...
...
│   ├── 大纲.md                # 全书卷级结构
│   ├── 卷纲/
│   │   └── 第1卷.md           # 每卷一个：对标结构坐标+剧情单元+情绪弧线(含章节定位)+人物弧线+伏笔+反转
│   ├── 角色线/
│   │   └── 沈栀.md            # 核心角色独立弧线计划：弧线定义+阶段规划+变化信号（状态机进 追踪/)
│   └── 细纲/
│       └── 第001章.md         # 每章一个：章节定位+事件+钩子(按章节定位,章首/章尾/段落级)+爽点+悬念
├── 正文/
│   ├── 第001章_章名.md
│   └── ...
├── 对标/                          ← 拆文产出的结构化资产
...
| 设定/文风.md（自定义文风·优先级最高） | 本书 | 用户自写（AI 可代写）；导入/拆解不覆盖 | Phase 4 每章写作前：含实质内容则取代对标文风作权威风格基 |
| 对标/{书名}/文风.md | 对标书 | analyze Stage 6 输出 → story-import 显式绑定或本 skill 首次引用时同步 | Phase 4 每章写作前（文风召回；有自定义文风时降为参考/句长兜底） |
| 大纲/卷纲/第X卷.md | 卷 | Phase 3 | Phase 4 写卷首章前 |
...
| 大纲/角色线/{线名}.md | 核心角色 | Phase 3 卷纲后建 + 登记 | 角色大节点/细纲「本章设定引用」时读计划（阶段/变化信号） |
| 追踪/角色线/{线名}.md | 核心角色（有独立线） | Phase 4 每章推进 | 细纲引用该线时读进度视图；`arc-audit` 查滞后 |
...
- **细纲一章一个文件**：`大纲/细纲/第XXX章.md`，含钩子设计，与正文一一对应
