...
4. **机械核验**（修订后必跑，工具层）：
   ...
   - `node scripts/check-chapter-consistency.js --project {项目根}`——时间线/星期/日期/倒计时
...
> 2. **正文硬事实回写**：逐章核对并回写——章序引用（正文内"第X章"自引用）、日期/星期（倒挂修复）、倒计时/剩余天数、"上一章/下一章"衔接表述——回写后跑 `check-chapter-consistency.js --project {项目根}` 验证归零。
