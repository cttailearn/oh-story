...
跨多卷有完整故事线的核心角色一人一卡（功能型配角并入主卡角色线，不单独建卡）。角色线 = 剧情规划的人物视角切片；**只写计划，实际进度以追踪为准**。

```markdown
# 角色线：{角色名}（{身份}）　线型：{主线|副线|感情线|成长线|事业线|自定义线型（如 调教线、复仇线、养娃线——直接写自定义线名，不限枚举）}

> 计划视图。实际进度以 `追踪/` 为准；本卡只写规划。阶段随剧情推进增量补写，细纲「本章设定引用」按阶段标题取用（如「角色线：苏黎线·多细胞整合」）。
...
创建/维护：Phase 3 搭完卷纲后按角色抽卡；细纲「本章设定引用」按阶段标题取用；日更只在角色大节点时按需读。
