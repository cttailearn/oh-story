...
## 角色卡内「剧情线」标题（原 大纲/角色线/ 已合并入角色卡）

> v1.5 起角色线与角色卡整合为一份文件：`设定/角色/{角色名}.md` 的「## 剧情线」大标题（不再建 `大纲/角色线/` 独立卡）。阶段小节式结构与旧角色线卡一致：

```markdown
## 剧情线

> 只写计划；实际进度以 追踪/ 与卷纲为准。阶段随剧情推进增量补写，
> 细纲「本章设定引用」按阶段标题取用（如「剧情线：{角色名}·阶段2」）。
...
创建/维护：Phase 3 搭完卷纲后在角色卡内补写「剧情线」标题（核心角色一人一份；功能型配角写「无独立线，并入 {主卡名}」）；细纲「本章设定引用」按阶段标题取用；日更只在角色大节点时按需读。完整 6 大标题模板见 character-basics.md 第 1 节。

**旧项目兼容**：存量项目的 `大纲/角色线/角色线_{名}.md` 继续可读（细纲引用、一致性检查照旧消费）；新卡一律用角色卡内「剧情线」标题，不再新建旧目录；迁移到新格式后删除旧文件即可。
