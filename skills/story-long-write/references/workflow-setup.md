...
**落盘后主线程机械校验**（grep 标题标记）：必查项——姓名/性别/角色定位/身份标签/性格关键词（含矛盾面）/核心目标/核心动机/致命弱点/口头禅/语言风格档案 7 维/成长弧线/能力上限/红线禁忌/项目专属设定/硬事实表（key: value 行，禁表格）/形象图/剧情线指针；**按启用维度校验**——卡头「特征启用」声明了哪些维度（含自定义维度），就必须有对应表。任一缺失 → 回退让 character-designer 补齐，**不静默接受**；跨时期/跨形态角色必须用分时期表表达，禁止压成单值字段。
