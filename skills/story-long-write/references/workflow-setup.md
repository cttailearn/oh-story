...
角色卡按 `character-basics.md`（主角卡模板，由 character-designer 自行解析）**整卡落盘**到 `设定/角色/{主角名}.md`（字段一一对应，含分时期身体/外貌特征表、硬事实表 key:value 单行、形象图字段、剧情线指针 `大纲/角色线_{名}.md`）。

**落盘后主线程机械校验必填字段**（grep 标题标记）：性别 / 身份标签 / 性格关键词（含矛盾面）/ 核心目标 / 核心动机 / 致命弱点 / 口头禅 / 身体特征表 / 外貌特征表 / 语言风格档案 7 维 / 成长弧线 / 能力上限 / 红线禁忌 / 项目专属设定 / 硬事实表（key: value 行）/ 形象图 / 剧情线指针。任一缺失或硬事实表用表格而非 key: value 单行 → 回退让 character-designer 补齐，**不静默接受**；跨时期/跨形态角色必须用分时期表表达，禁止压成单值字段。
