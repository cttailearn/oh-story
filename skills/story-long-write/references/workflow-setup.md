...
角色卡按 `character-basics.md`（主角卡模板，由 character-designer 自行解析）**整卡落盘**到 `设定/角色/{主角名}.md`（字段一一对应；特征按「特征维度库」裁剪启用——模板只预置最常用的**外形特征**（身体+外貌合并），其余能力/形态/身份/等级境界/道具/生态/关系按题材追加、库外用 `自定义{维度名: 列1/列2/...}`，需要哪个建哪个表，写在卡头「特征启用」行；跨时期/跨形态维度用分时期表；硬事实表 key:value 单行；形象图字段；剧情线指针 `大纲/角色线/{名}.md`）。
...
- 对跨多卷有完整故事线的核心角色（主角、主要反派、关键配角）一人一卡，抽 `大纲/角色线/{名}.md`（模板见 artifact-protocols.md「角色线卡」：**无表格的阶段小节式**——每阶段一个 `## 阶段 N：{阶段名}（{卷/章范围}）` 小节，内含 阶段目标/关键事件与场景（逐行列表，可随细纲增补）/同场角色/解锁伏笔/阶段情绪 + 与其他线交织点）；功能型配角不单独建线，并入主卡角色线
