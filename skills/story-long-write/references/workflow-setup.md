...
**细纲引用完整性检查（每批细纲落盘后执行，Gate C）**：对每章细纲「本章设定引用」行点名的角色卡/角色线/世界观/势力/物品，grep 确认对应设定文件存在（角色线引用 → 角色卡「角色线」标题存在）——缺 → 回补设定（按上方「细纲后设定补全」建档）或改细纲引用，不静默；引用存在但内容与细纲矛盾 → 修订细纲或补设定，二选一不并存。
