...
角色卡按 `character-basics.md`（6 大标题制模板，由 character-designer 自行解析）**整卡落盘**到 `设定/角色/{主角名}.md`（基础信息/形象与能力/说话方式/剧情线/写作红线/本书专属 6 大标题；硬事实 key:value 单行；形象图；剧情线阶段规划写卡内，不再建独立卡）。
...
**落盘后主线程机械校验**（grep 标题标记）：必查项——**H1 标题 = `# 角色卡：{名}（主角/女主/配角/反派/龙套）`（定位括号正确）**/**6 大标题齐全（主角/女主卡）**：基础信息（含核心目标/核心动机/致命弱点三项无 `[待补充]` + 硬事实 key: value 单行禁表格）/形象与能力/说话方式/剧情线/写作红线/本书专属。任一缺失 → 回退让 character-designer 补齐，**不静默接受**；其余字段的 `[待补充]` 必须带回补触发点+责任人，裸 `[待补充]` 视为缺失。
...
**人物弧线依据（写各卷人物弧线前必读）**：写卷纲「人物弧线」前，先读该角色 `设定/角色/{名}.md` 角色卡（形象与能力/写作红线/能力上限 + 「剧情线」标题的阶段蓝图/关键事件/阶段情绪），卷纲人物弧线按剧情线阶段对齐展开，禁止凭空写弧线。
...
卷纲「人物弧线」与细纲「本章设定引用·剧情线」都依赖剧情线阶段规划——**必须在细纲前写好**，不得等细纲后再补：

- 对跨多卷有完整故事线的核心角色（主角、主要反派、关键配角）在**角色卡内补写「## 剧情线」标题**（模板见 character-basics.md 第 1 节：**阶段小节式**——每阶段一个 `### 阶段 N：{阶段名}（{卷/章范围}）` 小节，内含 阶段目标/关键事件与场景（逐行列表，可随细纲增补）/同场角色/解锁伏笔/阶段情绪 + 与其他线交织点 + 进度指针）；功能型配角写「无独立线，并入主卡」
- 阶段蓝图覆盖到当前卷及后续至少一卷；**只写计划，实际进度以追踪为准**
- 建卡时回读角色卡「形象与能力」与卷纲「人物弧线」，三者对齐（不一致 → 先修角色卡/卷纲，再写剧情线）
...
- **核心角色剧情线阶段规划存在**（角色卡「剧情线」标题，阶段蓝图覆盖当前卷；卷纲「人物弧线」以此为根基）
- 大纲安全七检批次级答完 + 大纲安全审查通过

任一不达标 → **停靠**：列出缺项（如「第 2 卷卷纲未建」「剧情线缺：{名}」），用 AskUserQuestion 问「补齐再写细纲 / 带缺继续（标注风险）」；带缺继续时把缺项写入 `设定/题材定位.md`「设定待补」小节，后续滚动补纲前复查。**不静默带缺**。
...
- 本章设定引用：{角色卡:江晨/张耀祖；世界观:力量体系§传送阵；势力:文工团；剧情线:女主·阶段2（旧项目兼容 角色线:女主线-阶段2）；物品:龙血针——按本章实际用到填写，没有对应项写“无”；写作中发现用到清单外设定时按 workflow-chapter 步骤 2 补登记}
...
**细纲质量要求**：每章细纲都要按当前章节蓝图直接指导正文（阶段位置、结构公式、禁止提前释放、内容概括、情节安排、人物关系/出场顺序、情节细化、结尾设定齐备），但**强度按章节定位分配、不是每章顶满**：高压/推进章配齐钩子+爽点+悬念；低压/关系/修炼/信息整理章允许无显性爽点、弱钩子或仅情绪钩子，重点是把功能（喘息、关系、铺垫、转场）写到位。底线是每章都给读者一个往下看的理由、相邻章不情绪趋同（见 references/outline-structure-theory.md「章节定位与张弛」）。**写细纲前必读本章涉及角色的角色卡**：按「本章设定引用」加载 `设定/角色/{名}.md`（含「剧情线」标题当前阶段），细纲的目标情绪/人物关系变化/剧情线引用与剧情线阶段对齐。字段不完整时先补齐；无法从材料确定的关系或副线写 `[待补充]`，不得杜撰。
...
- 核心角色的剧情线阶段规划已在卷纲后写（见「角色卡剧情线标题」），此处只按细纲新信息**增量补充**其阶段蓝图；功能型配角不单独写线，并入主卡。

已存在的设定文件按细纲新信息**增量补充、不覆盖**。一次性路人、后文无戏份的配角不建静态档案或动态快照。建档只填细纲已确定的信息，未定字段留占位符，不提前杜撰。

**细纲引用完整性检查（每批细纲落盘后执行，Gate C）**：对每章细纲「本章设定引用」行点名的角色卡/剧情线/世界观/势力/物品，grep 确认对应设定文件存在（剧情线引用 → 角色卡「剧情线」标题存在；旧项目兼容 `大纲/角色线/` 旧文件）——缺 → 回补设定（按上方「细纲后设定补全」建档）或改细纲引用，不静默；引用存在但内容与细纲矛盾 → 修订细纲或补设定，二选一不并存。
...
- 任务：卷级结构、首批细纲、钩子/反转/情绪弧线。**写人物弧线与细纲前，必须读对应角色的 `设定/角色/{名}.md` 角色卡（含「剧情线」标题的阶段蓝图/关键事件/阶段情绪）**，弧线按剧情线阶段对齐，禁止凭空写。
