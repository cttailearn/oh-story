...
角色卡按 `character-basics.md`（主角卡模板，由 character-designer 自行解析）**整卡落盘**到 `设定/角色/{主角名}.md`（字段一一对应；特征按「特征维度库」裁剪启用——模板只预置最常用的**外形特征**（身体+外貌合并），其余能力/形态/身份/等级境界/道具/生态/关系按题材追加、库外用 `自定义{维度名: 列1/列2/...}`，需要哪个建哪个表，写在卡头「特征启用」行；跨时期/跨形态维度用分时期表；硬事实表 key:value 单行；形象图字段；剧情线指针 `大纲/角色线/角色线_{名}.md`）。

**落盘前先收集用户自定义特征需求**：询问「是否需要本书特有的特征维度？」（如女性角色身材特征：身高/三围/体重/体型；或战斗数值：力量/敏捷/灵力），把自定义清单（维度名 + 列名）传给 character-designer，写入卡头「特征启用」行 `自定义{维度名: 列1/列2/...}` 并建对应表（列头用户指定，首列固定「时期/阶段」）。

**落盘后主线程机械校验**（grep 标题标记）：必查项——**H1 标题 = `# 角色卡：{名}（主角/女主/配角/反派/龙套）`（定位括号正确）**/姓名/性别/角色定位/身份标签/性格关键词（含矛盾面）/核心目标/核心动机/致命弱点（**三项无 `[待补充]`**）/口头禅/语言风格档案 7 维/成长弧线/能力上限/红线禁忌/项目专属设定/硬事实表（key: value 行，禁表格）/形象图/剧情线指针；**按启用维度校验**——卡头「特征启用」声明了哪些维度（含自定义维度），就必须有对应表。任一缺失 → 回退让 character-designer 补齐，**不静默接受**；跨时期/跨形态角色必须用分时期表表达，禁止压成单值字段；其余字段的 `[待补充]` 必须带回补触发点+责任人，裸 `[待补充]` 视为缺失。
...
- `子代理`character-designer`（subagent 工具，task："项目目录：{dir}\n任务类型：角色设定\n查询参数：{主角设定信息}"`，有对标书时在 prompt 附 `对标书路径：{对标}/{书}/角色/{名}.md`）` — 辅助角色设定和语言风格档案（标题必须带定位括号；核心目标/动机/弱点不允许 [待补充]）
...
**人物弧线依据（写各卷人物弧线前必读）**：写卷纲「人物弧线」前，先读该角色 `设定/角色/{名}.md` 角色卡（成长弧线蓝图/红线禁忌/能力上限）与 `大纲/角色线/角色线_{名}.md` 角色线卡（阶段蓝图/关键事件/阶段情绪），卷纲人物弧线按角色线阶段对齐展开，禁止凭空写弧线。
...
卷纲「人物弧线」与细纲「本章设定引用·角色线」都依赖角色线卡——**必须在细纲前建好**，不得等细纲后再补：

- 对跨多卷有完整故事线的核心角色（主角、主要反派、关键配角）一人一卡，抽 `大纲/角色线/角色线_{名}.md`（模板见 artifact-protocols.md「角色线卡」：**无表格的阶段小节式**——每阶段一个 `## 阶段 N：{阶段名}（{卷/章范围}）` 小节，内含 阶段目标/关键事件与场景（逐行列表，可随细纲增补）/同场角色/解锁伏笔/阶段情绪 + 与其他线交织点）；功能型配角不单独建线，并入主卡角色线
- 阶段蓝图覆盖到当前卷及后续至少一卷；**只写计划，实际进度以追踪为准**
- 建卡时回读角色卡「成长弧线蓝图」与卷纲「人物弧线」，三者对齐（不一致 → 先修角色卡/卷纲，再落角色线卡）
...
- **核心角色角色线卡存在**，阶段蓝图覆盖当前卷（卷纲「人物弧线」以此为根基）
- 大纲安全七检批次级答完 + 大纲安全审查通过

任一不达标 → **停靠**：列出缺项（如「第 2 卷卷纲未建」「角色线卡缺：{名}」），用 AskUserQuestion 问「补齐再写细纲 / 带缺继续（标注风险）」；带缺继续时把缺项写入 `设定/题材定位.md`「设定待补」小节，后续滚动补纲前复查。**不静默带缺**。
...
- 本章设定引用：{角色卡:江晨/张耀祖；世界观:力量体系§传送阵；势力:文工团；角色线:女主线-阶段2；物品:龙血针——按本章实际用到填写，没有对应项写“无”；写作中发现用到清单外设定时按 workflow-chapter 步骤 2 补登记}
...
**细纲质量要求**：每章细纲都要按当前章节蓝图直接指导正文（阶段位置、结构公式、禁止提前释放、内容概括、情节安排、人物关系/出场顺序、情节细化、结尾设定齐备），但**强度按章节定位分配、不是每章顶满**：高压/推进章配齐钩子+爽点+悬念；低压/关系/修炼/信息整理章允许无显性爽点、弱钩子或仅情绪钩子，重点是把功能（喘息、关系、铺垫、转场）写到位。底线是每章都给读者一个往下看的理由、相邻章不情绪趋同（见 references/outline-structure-theory.md「章节定位与张弛」）。**写细纲前必读本章涉及角色的角色卡与角色线卡**：按「本章设定引用」加载 `设定/角色/{名}.md` 角色卡与 `大纲/角色线/角色线_{名}.md` 角色线卡（当前阶段行），细纲的目标情绪/人物关系变化/角色线引用与角色线卡对齐。字段不完整时先补齐；无法从材料确定的关系或副线写 `[待补充]`，不得杜撰。
...
- 角色 → 建 `设定/角色/{名}.md`（v2 填空模板见 character-basics.md 主角卡/配角卡：含语言风格档案、成长弧线蓝图、能力上限、红线禁忌、项目专属设定、硬事实表、剧情线指针）；核心复用角色真正进入正文后，由追踪事务建立 `追踪/角色状态/{名}.md` 动态快照；
...
- 核心角色的角色线卡已在卷纲后建（见「核心角色角色线卡」），此处只按细纲新信息**增量补充**其阶段蓝图；功能型配角不单独建线，并入主卡角色线。

已存在的设定文件按细纲新信息**增量补充、不覆盖**。一次性路人、后文无戏份的配角不建静态档案或动态快照。建档只填细纲已确定的信息，未定字段留占位符，不提前杜撰。

**细纲引用完整性检查（每批细纲落盘后执行，Gate C）**：对每章细纲「本章设定引用」行点名的角色卡/角色线/世界观/势力/物品，grep 确认对应设定文件存在——缺 → 回补设定（按上方「细纲后设定补全」建档）或改细纲引用，不静默；引用存在但内容与细纲矛盾 → 修订细纲或补设定，二选一不并存。
...
- 任务：卷级结构、首批细纲、钩子/反转/情绪弧线。**写人物弧线与细纲前，必须读对应角色的 `设定/角色/{名}.md` 角色卡与 `大纲/角色线/角色线_{名}.md` 角色线卡**（阶段蓝图/关键事件/阶段情绪），弧线按角色线阶段对齐，禁止凭空写。
