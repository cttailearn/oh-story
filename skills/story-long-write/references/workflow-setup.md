...
角色卡按 `character-basics.md`（主角卡模板，由 character-designer 自行解析）**整卡落盘**到 `设定/角色/{主角名}.md`（字段一一对应；特征按「特征维度库」裁剪启用——默认身体/外貌/语言/心理/特殊标记，按题材追加能力/形态/身份/等级境界/道具/生态/关系，写在卡头「特征启用」行；跨时期/跨形态维度用分时期表；硬事实表 key:value 单行；形象图字段；剧情线指针 `大纲/角色线_{名}.md`）。
