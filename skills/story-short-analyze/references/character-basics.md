...
硬事实表（grep 锚点；只写不变事实，可变状态归 追踪/角色状态/）：
  年龄: {}；身份: {}；等级: {}；称号: {}；所属势力: {}
...
- **静态/动态边界**：本卡只写不变事实与设计蓝图；位置/当前目标/身心状态/关系现状一律归 `追踪/角色状态/` 快照，剧情进度归 `大纲/角色线/` 卡——三处都写同一状态会互相矛盾
- 硬事实表用「key: value」单行写法，正文写作与一致性检查按行 grep 核对；表格里不放可变信息
...
标志性特征：（一句话让读者记住）
退场方式：（何时/如何退出故事）
硬事实表（grep 锚点）：身份: {}；与主角关系类型: {}；关键能力/资源: {}
