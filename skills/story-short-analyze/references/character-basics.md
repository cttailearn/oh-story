...
> **两个软锚点（下游机械消费，格式固定）**：①「基础信息」字段行——格式 `字段名：结论（补充描述）`，consistency-checker 取括号前结论值作事实锚点；②「角色线」内的 `### 阶段 N：{阶段名}（{卷/章范围}）` 小节标题（细纲「本章设定引用」按阶段标题定位）——**N 必须从 1 开始连续编号，不得跳号/重复**（中途插入阶段时整体重排后续编号；consistency-checker 会机械校验连续性）。
