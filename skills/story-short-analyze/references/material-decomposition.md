...
- **output-contract.md** = 输出契约 SSOT：定义 Stage → 文件映射 + `_meta.json` schema（含 `structure_counts`）+ 下游 `story-short-write` 消费规范 + 验收接入点。与 `story-short-write` 副本字节一致，由 `scripts/sync-shared-assets.py` 守护
