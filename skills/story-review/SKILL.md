...
name: story-review
version: 1.1.1
description: "多视角对抗式审查。full/lean 模式在已部署 reviewer agents 时并行 spawn；缺失/异常 agents 或 spawn 失败时自动降级 solo，参考文件不可读时使用内置 rubric fallback。触发方式：/skill:story-review、/story-review、/审查、「审查一下」「帮我审一下」。"
---
...
**story-explorer 预查询（可选）**。仅当 `Effective Mode` 仍为 `full`/`lean`、当前允许 spawn 且 subagent 工具可用时，才可检查 `.pi/agents/story-explorer.md` 是否存在并 spawn `story-explorer` 预查设定摘要；`solo` 或子代理递归保护场景下不得 spawn，只能直接 read/ffgrep。Prompt 示例：
