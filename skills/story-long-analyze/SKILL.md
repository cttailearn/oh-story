...
**预期耗时提示**：开始前根据章节数给用户一个粗估（**标准档**）：<50 章通常 15-30 分钟；50-200 章通常 40-80 分钟；>200 章可能需要多轮会话。全量档（用户要求全量拆解）用旧口径：<50 章 30-60 分钟；50-200 章 1-3 小时。Stage 2 可并行提取，Stage 3/4a/6 三路并行（见下），其余按依赖推进。
...
Stage 2 使用 chapter-extractor agent 处理章节：全量情节点章逐章并行，brief 章按批处理（每批一个 agent 处理多章，降低 spawn 固定成本），替代原来的串行分块。
...
- 分档表在执行前写入 `_progress.md`（列出哪些章全量、哪些 brief）；下游 `benchmark_style_load` 匹配章挑选只消费基调/情节点数/概括，brief 章够用；Stage 3-6 聚合时若某 brief 章成为关键证据，按需回补全量
...
**全量情节点章（逐章 spawn）**：用 subagent 工具 spawn 子代理 `chapter-extractor`（pi-subagents），task 传：
...
- **全量情节点章**：每章 1 个 agent，一批 5-8 个（pi-subagents 并发上限 4，一批实际分两轮跑完）
- 等待当前批次全部完成后，再 spawn 下一批
- 每批完成后更新 `_progress.md` 记录已处理章节

### Agent 输出收集

- **全量章**：agent 返回 markdown 格式的提取结果，主线程将 agent 输出写入 `章节/第{N}章_摘要.md`；收集所有 agent 的出场人物表，供 Stage 3 合并使用
...
1. **执行失败**（agent crash / 超时 / 空输出）→ 同模型（flash）重试 1 次（brief 批为**整批重试**；全量章为单章重试）
2. **质量失败**（输出落盘后跑质量检查，任一不达标）→ **分档处理**：
   - **全量情节点章**：跑 chapter-extractor.md「质量检查」12 条自检（典型：情节点 < 10、P 行缺白描、概要写成条目罗列或整段「因为…所以…」串联、类型/基调/主题标签超出枚举、`基调：` 漏全角冒号、角色名为昵称/通用称呼）→ **升级到 opencode-go/deepseek-v4-pro 重试 1 次**
