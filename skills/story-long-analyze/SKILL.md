...
- **一页摘要档**（其余全部章节）：每章只输出四字段——概要 ≤150 字、关键事件 3-5 个一句话事件（不受情节点格式约束）、基调序列（如 轻松→紧张）、章尾钩子类型（章尾13式取一）；落盘同一路径 `章节/第{N}章_摘要.md`，`_progress.md` 标 `summary_brief`
...
2. **质量失败**（输出落盘后跑质量检查，任一不达标）→ **分档处理**：
   ...
   - **brief 章（一页摘要）**：四字段齐全且事实可 grep 回原文即合格；不合格 → 同模型（flash）重试 1 次，**不升 pro**（brief 章无格式契约，flash 足够）
