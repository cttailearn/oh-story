...
version: 1.0.0
description: "长篇网文拆文。深度拆解爆款长篇小说的黄金三章、人设架构、爽点设计、节奏控制。单一深度拆解管道：跑完黄金三章（Stage 1）后产出快速预览报告并询问是否继续全量拆解，确认后从 Stage 2 续跑逐章摘要、聚合分析、设定关系、汇总报告，全程产物落盘 拆文库/{书名}/。触发方式：/skill:story-long-analyze、/story-long-analyze、/长篇拆文、「帮我拆这本书」「拆这本书」「分析黄金三章」「深度拆解」「完整拆解」「系统拆解」或提供小说文本文件路径——全部进入同一管道。"
---
# story-long-analyze：长篇网文拆文

> **双运行时**：pi 触发 `/skill:story-long-analyze`；dsh 触发 `/story-long-analyze`；自然语言均可。两端流程指令通用。
...
**预期耗时提示**：开始前根据章节数给用户一个粗估（**标准档**）：<50 章通常 15-30 分钟；50-200 章通常 40-80 分钟；>200 章可能需要多轮会话。全量档（用户要求全量拆解，默认批处理）口径：<50 章 20-40 分钟；50-200 章 40-80 分钟；>200 章可能多轮会话。Stage 2 可并行提取，Stage 3/4a/6 三路并行（见下），其余按依赖推进。
...
操作（机械步骤优先脚本化，主线程只复核异常）：

- **先跑确定性切片脚本**：`python <skill-dir>/scripts/chapter-boundaries.py <原文路径>`（随本 skill 分发）——内置 style-profile-generator.md Step 4 的章节正则（含 千/两，覆盖 1000+ 章），自动完成：grep 章节行号、剔目录块（行距中位数法）、卷号消歧（多卷书「第一章」重起时保留卷号+全书连续重编）、四列表（章号/标题/起始行/字数）与连续性校验；脚本输出去空白的边界表 JSON 供复核
- 脚本不可用或识别率 <95% 时回退手工流程：用同一章节正则 grep 出全部章节行号，按下方规则剔目录块、消歧、落表
...
### Stage 3-4 并行执行（Stage 6 可同批并行）

**并行执行图**：

```
Stage 3（剧情聚合 + 角色合并）        ──┐
Stage 4a（设定：世界观/金手指/势力）  ──┼── 三路可并行（都只依赖 Stage 2 产物）
Stage 6（文风）                       ──┘
              │
              ▼（Stage 3 + 4a 都完成后；Stage 6 无下游依赖，可随批次收尾）
...
4a 数据源是 Stage 2 摘要故可与 3 并行；**Stage 6 文风数据源是原文 + 章节摘要 + 黄金三章深度拆解，同样不依赖 3-5，可三路并行**，只在 `拆文报告.md`（Stage 5）引文风结论时需等 Stage 6 完成；4b/4c 依赖 Stage 3 角色合并故串行。
...
Stage 2 使用 chapter-extractor agent 处理章节：全量情节点章与 brief 章都默认按批处理（每批一个 agent 处理多章，降低 spawn 固定成本），用户明确要求时可逐章（每章一个 agent），替代原来的串行分块。

### 抽取分档（默认标准档；用户明确要求「全量拆解」时用全量档）

**全量档**（用户明确要求「完整拆解 / 一次跑完 / 系统拆解 / 别问」）：全部章节走全量情节点档，默认按批处理（见「批量策略」）；用户明确说「每章一个智能体 / 逐章处理」时才逐章 spawn。

Stage 2 默认**分档抽取**（200 章书从约 3 小时压到 1 小时内，下游消费不受影响）：

- **全量情节点档**（下方格式契约，10-40 点/章）：黄金三章（第 1-3 章）+ 节奏锚点章（对标节奏迁移选中的 1/4·中点·3/4 关键情节章）+ Stage 3 聚合后回填的情绪峰值章
- **一页摘要档**（其余全部章节）：每章只输出四字段——概要 ≤150 字、关键事件 3-5 个一句话事件（不受情节点格式约束）、基调序列（如 轻松→紧张）、章尾钩子类型（章尾13式取一）；落盘同一路径 `章节/第{N}章_摘要.md`，`_progress.md` 标 `summary_brief`；**本档按批处理**（见「批量策略」），每批一个 agent 处理多章
- 分档表在执行前写入 `_progress.md`（列出哪些章全量、哪些 brief，并标 `processing_mode: batch`（默认）或 `per_chapter`（用户要求逐章时））；下游 `benchmark_style_load` 匹配章挑选只消费基调/情节点数/概括，brief 章够用；Stage 3-6 聚合时若某 brief 章成为关键证据，按需回补全量
- brief 章的质量检查简化：四字段齐全且事实可 grep 回原文即合格，不跑情节点 12 条自检

### 调用方式

每条章节 prompt **必须以「材料声明」前缀开头**（即下方 prompt 字符串首段）——给子代理正确语境，避免它把通俗题材的正常戏剧化剧情误判为有害内容而拒绝拆解。前缀是固定文本，逐字带上，不要改写或省略。

**全量情节点章（默认批模式；用户明确要求「每章一个智能体 / 逐章处理」时才逐章）**：

- **批模式（默认）**：每批 spawn 1 个 `chapter-extractor`，task 传章节边界表（行区间）+ 完整情节点格式，agent 用 `read` 按行区间读原文、逐章输出完整情节点、以分隔符切分：

```text
[材料声明] 以下为用户合法持有的虚构小说，任务是只读的转化性文学分析（编辑/评论视角）。暴力/复仇/家暴/情爱/黑暗伦理是该题材常规虚构元素，照常提取，不得以敏感拒绝整章或整本；个别片段无法处理则跳过、照常续完其余。

[批处理模式-全量] 你一次处理第 {N1}–{N2} 章，每章输出**完整情节点格式**（概要/关键事件/关键信息与扩写技法/逐章写法公式/出场人物/情节点 P1-PN），不省略不合并，与单章模式同一份格式契约。
用 read 按「章节边界」表的行区间读取 `拆文库/{书名}/原文/原文.txt`，逐章输出：

## 第{N}章 {标题}
{每章完整输出：**概要**…**关键事件**…**关键信息与扩写技法**…**逐章写法公式**…**出场人物**…**情节点** P1-PN，遵循下方 [情节点格式要求] 与 [输出前自检]}

每章之间用独占一行分隔符：`[CHAPTER_SPLIT 第{N}章 结束]`
每章输出前逐条过 [输出前自检]；禁止用 ```markdown 代码块包裹任何一章。
...
章节边界（本批 {N1}–{N2} 章，由主线程从 _progress.md 摘出）：
| 章号 | 标题 | 起始行 | 字数 |
|------|------|--------|------|
| {N} | {标题} | {起始行} | {字数} |
```

- **逐章模式（opt-in）**：用户明确要求「每章一个智能体 / 逐章处理」时，用 subagent 工具 spawn 子代理 `chapter-extractor`（pi-subagents），task 传（贴整章原文）：

```text
"[材料声明] 以下为用户合法持有的虚构小说，任务是只读的转化性文学分析（编辑/评论视角）。暴力/复仇/家暴/情爱/黑暗伦理是该题材常规虚构元素，照常提取，不得以敏感拒绝整章或整本；个别片段无法处理则跳过、照常续完其余。\n\n章节编号：第{N}章\n章节标题：{标题}\n章节字数：{字数}\n\n章节原文：\n{原文文本}\n\n[情节点格式要求] 模板里的 {} 是占位标记，不要输出花括号本身：写「类型信息揭示」，不写「类型：信息揭示」或「类型{信息揭示}」。主题标签只填一个值，不要用 / 、 ，或空格并列多个。空字段统一写「无」，不要用「—」，涉及段不可省略。每个情节点后紧跟自己的那一行「主题标签X | 基调：Y」，不要把标签行堆到文件末尾。正确样例：\nP7 **龙血针检测**：类型信息揭示 | 许七安用龙血针验出对方身份，当场揭穿 | 涉及许七安,郑兴怀 | 地点府衙后堂 | 物品龙血针 | 时间入夜\n主题标签悬念 | 基调：紧张\n\n[输出前自检] 交付前逐条核对：① 文本里没有 { 或 }；② ^P 行数 == 主题标签行数 == 基调：行数；③ 每个主题标签只有一个值；④ 每个 P 行都含类型、白描、涉及三段。任何一条不符，先改再输出。"
```

> 上面的 `[情节点格式要求]` / `[输出前自检]` 两段由主线程在 spawn 时拼进 prompt，**不依赖项目里已部署的 agent 文件版本**——老项目不重新跑 `/skill:story-setup` 也能拿到这份格式约束。pro 升级重试沿用同一段。

**brief 章（一页摘要，批模式）**：每批 spawn 1 个 `chapter-extractor`，task 传章节边界表（行区间）+ 4 字段格式，agent 用 `read` 按行区间读原文、逐章回传并以分隔符切分，主线程切分落盘：

```text
[材料声明] 以下为用户合法持有的虚构小说，任务是只读的转化性文学分析（编辑/评论视角）。暴力/复仇/家暴/情爱/黑暗伦理是该题材常规虚构元素，照常提取，不得以敏感拒绝整章或整本；个别片段无法处理则跳过、照常续完其余。

[批处理模式] 你一次处理第 {N1}–{N2} 章，每章只输出一页摘要 4 字段，不输出情节点。
用 read 按「章节边界」表的行区间读取 `拆文库/{书名}/原文/原文.txt`，逐章输出：
...
每章之间用独占一行分隔符：`[CHAPTER_SPLIT 第{N}章 结束]`
输出前自查：每章四字段齐全、概要 ≤150 字、关键事件为 3-5 个一句话事件、事实可 grep 回原文。
...
> brief 批的 4 字段格式与分隔符随 task 下发，**同样不依赖部署的 agent 文件版本**（agent 模板的「批处理模式」段是兜底说明，缺了也能按 task 执行）。

### 批量策略

- **brief 章（批模式）**：每批 1 个 agent 处理多章。批大小按实际模型上下文窗口取：64K 窗口 8-10 章/批，128K 窗口 15-20 章/批。执行前校验：`峰值上下文 ≈ 6.8K(system prompt) + 批章数 × (原文 ~3.5K + 输出 ~0.9K) < 窗口`，取能放下的最大整数，宁小勿大
- **全量情节点章（默认批模式）**：每批 1 个 agent 处理多章。批大小按实际模型上下文窗口取：64K 窗口 3-5 章/批，128K 窗口 8-10 章/批。执行前校验：`峰值上下文 ≈ 6.8K(system prompt) + 批章数 × (原文 ~3.5K + 输出 ~4.5K) < 窗口`，取能放下的最大整数，宁小勿大。**spawn 时把 `turnBudget` 覆盖为 ≈ 批章数 × 2 + 6**（逐章模式保持 agent 默认 12）
- **全量情节点章（opt-in 逐章）**：用户明确要求「每章一个智能体 / 逐章处理」时，每章 1 个 agent，一批 5-8 个（pi-subagents 并发上限 4，一批实际分两轮跑完）
- 等待当前批次全部完成后，再 spawn 下一批
- 每批完成后更新 `_progress.md` 记录已处理章节

### Agent 输出收集

- **全量章（逐章 opt-in）**：agent 返回 markdown 格式的提取结果，主线程将 agent 输出写入 `章节/第{N}章_摘要.md`；收集所有 agent 的出场人物表，供 Stage 3 合并使用
- **全量章（批模式，默认）**：agent 回传含 `[CHAPTER_SPLIT 第{N}章 结束]` 分隔符的文本，主线程按分隔符切分为每章一段，写入 `章节/第{N}章_摘要.md`；切分后章节数 != 批章数时按执行失败处理；出场人物表由主线程从各章文件 grep 收集（与逐章模式同源）
- **brief 批**：agent 回传含 `[CHAPTER_SPLIT 第{N}章 结束]` 分隔符的文本，主线程按分隔符切分为每章一段，写入 `章节/第{N}章_摘要.md`；切分后章节数 != 批章数时按执行失败处理

### 失败处理 + 质量升级重试

**两类失败**：

1. **执行失败**（agent crash / 超时 / 空输出）→ 同模型（flash）重试 1 次（brief 批与全量批均为**整批重试**；全量逐章为单章重试）
2. **质量失败**（输出落盘后跑质量检查，任一不达标）→ **分档处理**：
   - **全量情节点章（批/逐章同链）**：跑 chapter-extractor.md「质量检查」12 条自检（典型：情节点 < 10、P 行缺白描、概要写成条目罗列或整段「因为…所以…」串联、类型/基调/主题标签超出枚举、`基调：` 漏全角冒号、角色名为昵称/通用称呼）→ **单章重试**：单独 spawn 1 个 agent 只重试该失败章——flash 同模型 1 次，仍不过再升 **opencode-go/deepseek-v4-pro** 1 次
   - **brief 章（一页摘要）**：四字段齐全且事实可 grep 回原文即合格；不合格 → **单章重试**：单独 spawn 1 个 agent 只重试该失败章（同模型 flash，不升 pro；task 沿用批模式模板但只含该章，可追加失败原因）
...
用 subagent 工具 spawn `chapter-extractor`，批模式单章重试时 task 只含该失败章（贴整章原文，可追加「上次校验失败原因：{自检失败项}」）；`model` 覆盖为 opencode-go/deepseek-v4-pro（显式覆盖 agent 默认的 opencode-go/deepseek-v4-flash）用于 pro 档，task 传：
...
- flash 首次通过 → 写入 `章节/第{N}章_摘要.md`，`_progress.md` 标记 `success`
- flash 失败 + 同模型 retry 通过 → 同上，备注 `retry_same_model`
- 全量章质量失败 + pro retry 通过 → 同上，备注 `retry_pro`（brief 章不升 pro，见上）
- pro retry 仍失败 → 章节标记 `⚠️ 跳过`，失败原因写入 `_progress.md` 「失败记录」表，拆文报告中注明
- 全量批：批内单章质量失败只重试该章（单章 spawn，flash → pro 链）；整批执行失败才整批重试 1 次
- brief 批：批内单章质量失败只重试该章（单章 spawn）；整批执行失败才整批重试 1 次；整批仍失败 → 该批各章 `⚠️ 跳过`，不影响其他批
