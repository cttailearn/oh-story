...
const LONG_PROJECT_DIRECTORY_MARKERS = new Set([
	"正文",
	"大纲",
	"设定",
	"追踪",
...
function shouldIgnoreDirectory(name) {
	return (
		IGNORED_DIRECTORIES.has(name) || (name.startsWith(".") && name !== ".story")
	);
}
...
export async function resolveWorkspaceDirectory(root, requestedPath) {
	...
	if (
		...
		requestedPath
			.split(/[\\/]+/)
			.some((segment) => shouldIgnoreDirectory(segment))
	) {
		throw new DashboardError(
			403,
			"directory_hidden",
			"该目录不会显示在 Dashboard 中",
		);
	}
...
export async function listWorkspaceDirectory(
	...
	const { absolutePath, realRoot } = await resolveWorkspaceDirectory(
		root,
		requestedPath,
	);
		...
		await Promise.all(
			page.map(async (entry) => {
				...
				return {
					...
					type: "file",
					editable:
						isEditableFile(childAbsolute) && info.size <= MAX_FILE_BYTES,
					size: info.size,
				};
			}),
		)
		...
		for (const entry of entries) {
			if (entry.isDirectory() && !entry.isSymbolicLink()) {
				roots.push({
					...
					relativePath: `拆文库${sep}${entry.name}`,
				});
			}
		}
	}
...
async function findProjectRoots(
	...
	const isShortProject =
		...
		[...SHORT_PROJECT_COMPANION_FILES].some((marker) =>
			childFileNames.has(marker),
		);
...
export async function scanWorkspace(root) {
	const realRoot = await existingRealRoot(root);
	const { libraryRoots, projectRoots, scanErrors } =
		await discoverWorkspaceRoots(realRoot);
...
async function readWorkspaceFile(root, requestedPath) {
	const { absolutePath, info } = await resolveWorkspacePath(
		root,
		requestedPath,
		{
			editableOnly: true,
...
async function replaceFileAtomically(target, content, mode) {
	const temporary = resolve(
		dirname(target),
		`.${basename(target)}.story-dashboard-${randomUUID()}.tmp`,
	);
	...
	try {
		...
		if (
			...
			!["EACCES", "EPERM", "EEXIST"].includes(error?.code)
		) {
...
async function saveWorkspaceFile(root, payload) {
	...
	return withSerializedFileMutation(initial.absolutePath, async () => {
		let current;
		try {
			current = await resolveWorkspacePath(root, payload.path, {
				editableOnly: true,
			});
		} catch (error) {
...
async function serveStaticFile(requestPath, response) {
	...
	response.writeHead(
		200,
		responseHeaders(
			CONTENT_TYPES[extname(assetName)] || "application/octet-stream",
		),
	);
	response.end(body);
}
		...
		try {
			...
			if (request.method === "PUT" && url.pathname === "/api/file") {
				sendJson(
					response,
					200,
					await saveWorkspaceFile(workspaceRoot, await readJsonBody(request)),
				);
				return;
			}
			if (request.method === "DELETE" && url.pathname === "/api/file") {
				sendJson(
					response,
					200,
					await deleteWorkspaceFile(workspaceRoot, await readJsonBody(request)),
				);
				return;
			}
...
async function listen(server, host, preferredPort) {
	const attempts =
		preferredPort === 0
			? [0]
			: Array.from({ length: 11 }, (_, index) => preferredPort + index);
