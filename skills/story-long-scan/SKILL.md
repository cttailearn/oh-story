...
**采集失败降级（反爬更新/脚本失效时，不阻塞扫榜）**：脚本报错、解析率异常（文件头 `数据质量` 为「存在问题」且修复无效）或平台改版时，按以下顺序降级：

1. 让用户粘贴榜单页**截图** → 视觉解析（pi 图像输入或 agent_browser 截图）提取条目，标注 `[截图解析]` 与采集日期
2. 用户粘贴榜单文字/表格 → 手工结构化，标注 `[手工录入]`
3. 仍不可得 → 转入「内置知识」模式并在报告开头声明：以下基于历史趋势数据，未经实时校验
