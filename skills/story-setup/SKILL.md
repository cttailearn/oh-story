...
- 已存在 → 只处理「网文写作工具集」段，段识别规则（重要，旧版段名需兼容，已实测）：
  - 段起点：标题文本匹配 `#{1,2} .*网文写作工具集(（pi）)?`（新版 `# {项目名} — 网文写作工具集`，
    兼容旧版 `# {项目名} — 网文写作工具集（pi）` 及手写 H2 变体）。
  - 段结束：起点之后**最后一个模板子节**（「Skill 路由表」「文件结构」「协作规则」
    「Compact 后恢复上下文」四个 `## ` 标题）的内容结束处；子节之外、段之后的内容（用户的
    自定义 `#`/`##` 段、其他文件约定）一律原样保留，不得吞并或删除。
  - 找到该段 → 用模板（替换 {项目名} 后）的对应段**整体替换**；未找到 → 模板整份**追加到文件末尾**。
