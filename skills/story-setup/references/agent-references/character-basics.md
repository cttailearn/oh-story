...
身体特征（分时期/阶段；设定完善时在对应时期行下直接修改，新阶段出现时加行，不另起字段）：
...
外貌特征（分时期；规则同上——人类时期/穿越后/化形后等各有独立行）：
| 时期/阶段 | 外貌特征（3-5 个关键词，要有记忆点） | 记忆点/备注 |
|-----------|-----------------------------------|------------|
| {如：人类时期} | {如：清瘦、黑框眼镜、熬夜黑眼圈} | {如：总盯着动植物看} |
| {如：细胞体时期} | {如：半透明、可变形、随吞噬改变形状} | {如：形态即生存状态} |

> **分时期特征规则**：特征表只记**永久性形态/阶段事实**（少年→青年、人类→细胞体、化形等）。设定完善时在对应时期行下直接修改或加行，**不另起字段、不另建文件**；临时状态（当前位置/伤势/情绪/当前目标）一律归 `追踪/角色状态/` 快照，不进本表。正文描写前 narrative-writer 读本表取**当前时期行**。
...
身体/外貌特征：{需要时用分时期表，规则同主角卡；普通配角一句话即可}
退场方式：（何时/如何退出故事）
