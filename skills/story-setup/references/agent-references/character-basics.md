...
特征启用（按本书题材定位 + 用户需求裁剪，见「特征维度库」）：身体、外貌、{按题材追加：能力/形态/身份/等级境界/道具/生态/关系}{ + 自定义{身材特征: 身高/三围/体重/体型}（库外维度，列由用户指定）}
...
【自定义维度】（用户指定维度名 + 表列；首列固定「时期/阶段」，其余列用用户指定的列名）：
| 时期/阶段 | {列1} | {列2} | {列3} | 备注 |
|-----------|-------|-------|-------|------|
| {如：人类时期} | {如：身高 168cm} | {如：三围 88/62/90} | {如：体重 52kg} | {如：仅女性角色启用} |
| {如：细胞体时期} | {如：无固定尺寸} | {如：—} | {如：随吞噬变化} | {如：跨形态后行} |
