...
> **两个软锚点（下游机械消费，格式固定）**：①「基础信息」末尾的硬事实区 `key: value` 单行（consistency-checker 按行 grep）；②「角色线」内的 `### 阶段 N：{阶段名}（{卷/章范围}）` 小节标题（细纲「本章设定引用」按阶段标题定位）。

### 6 大标题与填写什么

| 标题 | 填写什么 |
| --- | --- |
| `## 基础信息` | 姓名/性别/年龄/角色定位/身份标签/性格关键词（含矛盾面）/核心目标/核心动机/致命弱点/口头禅与标志动作；末尾硬事实区：`key: value` 单行（只写不变事实；**建议 key**：姓名/性别/年龄/身份/能力/关系结论，自定义 key 沿用已有卡的写法，不另起别名；可变状态归 `追踪/角色状态/`） |
...
硬事实（key: value 单行，只写不变事实；建议 key：姓名/性别/年龄/身份/能力/关系结论）：
  姓名: …
  年龄: …
  身份: …
  能力: …
...
- 硬事实区（基础信息内 `key: value` 行）勿用 markdown 表格——consistency-checker 第零步只 grep `key: value` 行，表格会让整层机械核对静默跳过
