...
| `arc_status` | 查某条角色线进度 | "沈栀的角色线进行到哪了？" |
| `arc_progress` | 列出各角色线进度/滞后/已完结 | "各角色线进度如何？有该推进没推进的吗？" |
...
├── 设定/
│   ├── 世界观/          # 设定详情
│   ├── 角色/            # 角色文件（每个角色一个 .md；主角卡「角色线」标题=线指针）
│   ├── 势力/            # 势力/组织文件
│   ├── 关系.md          # 角色关系映射
│   └── 题材定位.md      # 题材定位
├── 大纲/
...
│   ├── 卷纲/
│   │   └── 第X卷.md    # 每卷规划
│   ├── 角色线/          # 独立角色线计划文件（{线名}.md：弧线定义+阶段规划+变化信号）
│   └── 细纲/
│       └── 第XXX章.md  # 每章蓝图
├── 正文/
│   └── 第XXX章_*.md     # 正文章节
...
### arc_status / arc_progress 流程

1. 校验 `追踪/上下文.md` 头部的 `状态修订：{N}`（同 character_status 第 1 步）；缺失/不一致时在 `gaps` 返回 `tracking_state_invalid`。
2. `arc_status`：给定线名，先 `fffind 大纲/角色线/{线名}.md` 确认存在（无独立线文件 → `gaps.no_line_file`，回退读角色卡「角色线」标题作计划），再 `read 追踪/角色线/{线名}.md` 取进度视图；汇总：当前阶段（进度视图 `当前：阶段 N/M · 进行中` 或 `已完结`）、已完成阶段证据锚点、下一阶段变化信号（读 `大纲/角色线/{线名}.md` 对应阶段）。
3. `arc_progress`：`fffind 大纲/角色线/*.md` 列全部线，逐线 `read 追踪/角色线/{线名}.md`，返回各线的 status/当前阶段/证据数；把「计划区间终点已过但未推进」的线标记为 `overdue`（数值以 `tracking_commit.py arc-audit` 输出为准，本 agent 只读视图做展示）。
4. 需要变化原因时定点 `ffgrep 追踪/逐章记录/` 对应章节，不回扫全历史。
5. 返回结构化结果；进度视图与计划文件冲突时如实报告两处内容，不自行改写。
...
4. 从细纲和续写状态卡提取角色名，读取 `设定/角色/{name}.md`；久别核心角色再读取 `追踪/角色状态/{name}.md`。若 `大纲/角色线/{name}.md` 存在（该角色有独立线文件），同时读取 `追踪/角色线/{name}.md` 进度视图 + 在线文件中取当前阶段「变化信号」——写正文要照着落实。
