...
### 第零步：硬事实表 diff（机械核对，先于术语扫描）

先做确定性事实核对，不依赖推理：

1. 读 `设定/角色/`、`设定/势力/`、`设定/世界观/` 各卡的硬事实区（角色卡=「基础信息」标题内的 `key: value` 行；势力/世界观卡=「硬事实表」小节）；无 `key: value` 行的旧卡跳过本步，不反向兼容推断
...
4. **角色卡内「角色线」阶段 ↔ 「形象与能力」分时期表对齐**（主角/女主卡）：读 `设定/角色/{名}.md` 的「形象与能力」分时期行（首列时期，如 高中/大学/异界卷）与「角色线」的 `### 阶段 N：` 标题；比对：卡内每个时期在「角色线」阶段标题/正文中应有对应（阶段标题含时期或对应卷名）；有时期但阶段完全没覆盖（阶段数 < 时期数且无“待后续卷”说明）→ 提示补阶段，不判错（剧情线是计划视图，可先缺后补）；时期与阶段**明显矛盾**（如卡内无异界时期、剧情线却有异界阶段）→ 记 `[S2][period_align]`；**旧项目兼容**：存量 `大纲/角色线/角色线_{名}.md` 照旧消费，比对时先看角色卡内「角色线」，无该标题再读旧路径
5. 输出格式：`[S2][hard_fact] 事实:{}；正文:{}；出处:设定/{路径} + 正文/{文件}`
