...
4. **角色卡内「角色线」阶段 ↔ 「形象与能力」分时期表对齐**（主角/女主卡）：读 `设定/角色/{名}.md` 的「形象与能力」分时期行（首列时期，如 高中/大学/异界卷）与「角色线」的 `### 阶段 N：` 标题；比对：卡内每个时期在「角色线」阶段标题/正文中应有对应（阶段标题含时期或对应卷名）；有时期但阶段完全没覆盖（阶段数 < 时期数且无“待后续卷”说明）→ 提示补阶段，不判错（角色线是计划视图，可先缺后补）；时期与阶段**明显矛盾**（如卡内无异界时期、角色线却有异界阶段）→ 记 `[S2][period_align]`
5. **角色卡内「角色线」阶段编号连续性**（主角/女主/反派卡）：用 ffgrep 提取 `设定/角色/{名}.md`「角色线」标题下所有 `### 阶段 N：` 标题的 N；N 必须从 1 开始、无跳号、无重复（插入阶段导致后续编号断裂会让细纲「本章设定引用·角色线:{名}·阶段N」失配）→ 不连续记 `[S1][stage_numbering]`，列出实际编号序列并建议重排（如 `阶段 1,2,4` → 重排为 `1,2,3`）
