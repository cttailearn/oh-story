...
4. **角色卡↔角色线卡时期对齐**（主角/女主卡）：读 `设定/角色/{名}.md` 特征表的时期行（`| {时期} |` 首列，如 高中/大学/异界卷），读 `大纲/角色线/角色线_{名}.md` 的 `## 阶段 N：` 标题；比对：角色卡每个时期在角色线卡阶段标题/正文中应有对应（阶段标题含时期或对应卷名）；角色卡有时期但角色线卡完全没覆盖（阶段数 < 时期数且无“待后续卷”说明）→ 提示补角色线阶段，不判错（角色线是计划视图，可先缺后补）；角色卡时期与角色线阶段**明显矛盾**（如角色卡无异界时期、角色线却有异界阶段）→ 记 `[S2][period_align]`
