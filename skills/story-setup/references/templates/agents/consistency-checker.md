...
1. 读 `设定/角色/`、`设定/势力/`、`设定/世界观/` 各卡的「硬事实表」小节（`key: value` 行）；无该小节的旧卡跳过本步，不反向兼容推断
...
4. **角色卡内「剧情线」阶段 ↔ 「形象与能力」分时期表对齐**（主角/女主卡）：读 `设定/角色/{名}.md` 的「形象与能力」分时期行（首列时期，如 高中/大学/异界卷）与「剧情线」的 `### 阶段 N：` 标题；比对：卡内每个时期在「剧情线」阶段标题/正文中应有对应（阶段标题含时期或对应卷名）；有时期但阶段完全没覆盖（阶段数 < 时期数且无“待后续卷”说明）→ 提示补阶段，不判错（剧情线是计划视图，可先缺后补）；时期与阶段**明显矛盾**（如卡内无异界时期、剧情线却有异界阶段）→ 记 `[S2][period_align]`；**旧项目兼容**：存量 `大纲/角色线/角色线_{名}.md` 照旧消费，比对时先看角色卡内「剧情线」，无该标题再读旧路径
