...
- 主角卡（6 大标题制）：「基础信息」（姓名/定位/性格/目标/动机/弱点/口头禅，字段格式 `字段名：结论（补充）`）、「形象与能力」（外貌记忆点/分时期表/能力上限/形象图）、「说话方式」（7 维+例句）、「角色线」（阶段规划/交织点/进度指针——与角色线整合，不再建独立卡）、「写作红线」、「本书专属」；模板见 character-basics.md 第 1 节，字段自由填写但 6 大标题主角卡必全
...
- 配角卡（中密度）：角色功能（导师/盟友/情报源/牺牲品/镜像对照）、与主角关系、核心特质（1-2个）、标志性特征（跨形态用 `时期: 特征` 分行）、退场方式、关键能力/资源（简写，详述归「形象与能力」）；**加写**角色线（无独立线写并入主卡）；6 大标题按功能裁剪只写相关标题
...
- **静态/动态边界**：落盘时只写不变事实与设计蓝图；位置/目标/状态类信息不入卡（归追踪快照），阶段事件/场景写卡内「角色线」标题（**只写计划**，进度以追踪/卷纲为准）
