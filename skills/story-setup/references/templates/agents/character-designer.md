...
- 主角卡：姓名、性别、角色定位、身份标签、性格关键词（须有矛盾面）、核心目标、核心动机（情感驱动）、致命弱点、口头禅/标志动作；**加写**特征表（按「特征维度库」启用维度逐项建分时期表）、语言风格档案（7维结论+例句）、成长弧线设计（三阶段蓝图）、能力/金手指上限（交叉引用题材定位）、红线与禁忌、项目专属设定、硬事实表（key: value 单行）、形象图、剧情线指针（`大纲/角色线/角色线_{名}.md`）
