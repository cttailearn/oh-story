...
- 主角卡（6 大标题制）：「基础信息」（姓名/定位/性格/目标/动机/弱点/口头禅+硬事实 key:value 单行）、「形象与能力」（外貌记忆点/分时期表/能力上限/形象图）、「说话方式」（7 维+例句）、「剧情线」（阶段规划/交织点/进度指针——与角色线整合，不再建独立卡）、「写作红线」、「本书专属」；模板见 character-basics.md 第 1 节，字段自由填写但 6 大标题主角卡必全
...
- **特征维度按需启用（模板只预置最常用的外形特征）**：落盘前按本书题材（参考 `设定/题材定位.md`）与用户需求裁剪启用清单，写在卡头「特征启用」行——默认外形特征（身体+外貌合并，必填），按题材追加能力/形态/身份/等级境界/道具/生态/关系，库外维度用 `自定义{维度名: 列1/列2/...}`；语言/心理/特殊标记不单独建特征表（已并入 7 维档案/性格关键词/外貌记忆点）；**不是所有项目都要写全维度，需要哪个建哪个表**
...
- **静态/动态边界**：落盘时只写不变事实与设计蓝图；位置/目标/状态类信息不入卡（归追踪快照），阶段事件/场景写卡内「剧情线」标题（**只写计划**，进度以追踪/卷纲为准）
