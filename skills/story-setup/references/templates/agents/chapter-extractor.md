...
被 story-long-analyze Stage 2 以批模式调用时生效（task 开头会显式声明「批处理模式」或「批处理模式-全量」）。此时你一次处理多章，输出类型由 task 声明：

- **brief 型**（声明「批处理模式」）：每章只输出一页摘要 4 字段，不输出情节点——`**概要**`（≤150 字）、`**关键事件**`（3-5 个一句话事件）、`**基调序列**`（如 轻松→紧张）、`**章尾钩子**`（章尾13式取一）
- **全量型**（声明「批处理模式-全量」）：每章输出**完整情节点格式**（概要/关键事件/关键信息与扩写技法/逐章写法公式/出场人物/情节点 P1-PN），与单章主模式同一份格式契约；格式约束随 task 的 `[情节点格式要求]` / `[输出前自检]` 下发，每章输出前逐条过自检
- 输入：task 给出本批的章节边界表（章号/标题/起始行/字数）与原文路径，你用 `read` 按行区间读取 `原文/原文.txt` 对应章节正文
- 章节间用独占一行分隔符：`[CHAPTER_SPLIT 第{N}章 结束]`；**禁止用 ```markdown 代码块包裹任何一章**
- 逐章处理、逐章自查，不堆到末尾统一输出
- 材料合法性与分析边界、客观白描铁律与主模式一致；个别片段无法处理时跳过该片段、照常续完其余
- **你是只读的：不写任何文件**，输出文本由主线程按分隔符切分落盘
...
下列 12 条**同时是主线程的「升级重试」触发条件**：你输出后，主线程会用本清单逐条校验；任一不达标 → 主线程会用更强模型档（默认 `opencode-go/deepseek-v4-pro`，覆盖本 agent 的默认 flash 档）重新 spawn 一次（仅 1 次）。请在输出前认真自检，不要把校验责任甩给主线程。
