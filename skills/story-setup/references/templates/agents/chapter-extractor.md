...
被 story-long-analyze Stage 2 以批模式调用时生效（task 开头会显式声明「批处理模式」）。此时你一次处理多章，每章只输出**一页摘要（4 字段）**，不输出情节点：

- 输入：task 给出本批的章节边界表（章号/标题/起始行/字数）与原文路径，你用 `read` 按行区间读取 `原文/原文.txt` 对应章节正文
- 每章输出固定 4 字段：`**概要**`（≤150 字）、`**关键事件**`（3-5 个一句话事件）、`**基调序列**`（如 轻松→紧张）、`**章尾钩子**`（章尾13式取一）
- 章节间用独占一行分隔符：`[CHAPTER_SPLIT 第{N}章 结束]`
- 逐章处理、逐章自查（四字段齐全且事实可 grep 回原文即合格），不堆到末尾统一输出
