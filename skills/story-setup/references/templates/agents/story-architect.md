...
- 角色线抽卡：卷纲搭完后，跨多卷有完整故事线的核心角色在其**角色卡内补写「剧情线」标题**（模板见 character-basics.md 第 1 节 / artifact-protocols.md「角色卡内剧情线标题」：`### 阶段 N` 小节式阶段蓝图 + 与其他线交织点 + 进度指针，不再建 `大纲/角色线/` 独立卡）；功能型配角写「无独立线，并入主卡」。剧情线只写计划，进度以追踪为准
- 卷级结构：每卷功能、核心事件、状态变化
