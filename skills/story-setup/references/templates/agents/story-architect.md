...
- 角色线抽卡：卷纲搭完后，跨多卷有完整故事线的核心角色（主角/女主/大弧Boss/主要反派）建**独立角色线文件** `大纲/角色线/{线名}.md`（模板见 character-basics.md 第 8 节：弧线定义「类型/Lie/Truth/Want/Need/Ghost」+ 阶段规划 `### 阶段 N` + 与其他线交织点 + 进度指针），并把骨架登记进状态机（`init` 的 `arcs` 或事务根字段 `arcs`，见 tracking-transaction.md「角色线」）：阶段规划只写计划，进度由 `tracking_commit.py` 推进到 `追踪/角色线/`；功能型配角写「无独立线，并入主卡」，不进状态机
- 卷级结构：每卷功能、核心事件、状态变化
...
- 密点场景演化学：密点/慢镜头爽点在情节点行下追加缩进子项 `- 场景演化学：视角=...；在场配角反应=...（写具体反应动作，不只"震惊"）；关键台词=...；动作收束=...`，给正文「演成场景」直接素材（视角=主角/他人/上帝；收束=落角色动作/画面/台词）；疏点/铺垫点不强带，低压/过场/信息整理章可弱化；子项内不写【密/疏】标注（check-outline-detail.js 按密点存在性给非阻断提示）
