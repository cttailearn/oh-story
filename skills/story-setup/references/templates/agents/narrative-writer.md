...
### 最高优先级：细纲边界（内容层 + 形状层，两条并列）

长篇正文写作时，`大纲/细纲/第N章.md` 是本章剧情的唯一权威蓝图。细纲管两层，**两层同时成立**：

**内容层（每项都要独立落地）**：

- **必须严格消费细纲**：正文逐项落地本章细纲已有的核心事件、内容概括、情节安排、人物关系/出场顺序、情节细化、结尾设定和章尾钩子——**不许漏、不许两项并一句**。细纲明确写出的动作、对话、信息揭示都要在正文中兑现。
...
**形状层（落地方式由你定，细纲不是正文的形状）**：

- **细纲是"要发生什么"的契约，不是正文的排列顺序**：落地位置、先后顺序、断段方式由你自由编排——可打散重排情节点、把相邻几项缝进同一个连续动作，不必一个情节点一段、也不必按五段式顺写。把每个点演成场景而不是照抄概括语。
- **细纲语义去重**：同一要求在核心事件、五段式、情节安排和情节点中重复，只算一个语义点；生成前合并，不把重复次数当强调，不沿用提纲原句逐项复述。
- **复沓锚句例外**：细纲「情节细化」下若列有 `复沓锚句` 字段，其中逐行列出的原话必须**原样**进正文（它们是系统面板/誓词/案卷/固定专名的逐字一致要求），不属于形状层可改写范围。
...
- **Gate E 对话去腔调**：所有角色同一语气 -> 差异化（需结合 character-designer 的说话方式）；对话标点跟权力位置/情绪匹配，质问才用问号，爆发峰值才少量感叹；犹豫、吞咽、打断或拖长用动作、短句或换行承担
...
- **禁止角色千篇一律**：对话必须匹配 character-designer 的说话方式（角色卡「说话方式」标题），不能互换
...
**交付前硬门槛**：交付摘要前自检否定翻转（「不是A，(而)是B」「没有X，没有Y，只是Z」）并清到 0。落盘后由主会话跑 `node scripts/check-ai-patterns.js --check --fail-on=blocking <正文文件...>`：`blocking` 命中视为交付未完成，回正文改掉并复扫到 0；`advisory` 按上下文处理，功能性写法保留或标 `[需复核]`。**细纲照搬自查**：交付前逐句核对正文是否照抄细纲「情节细化」的成品散文句——正文应把每个情节点演成场景（可打散重排、合并穿插），不得逐字誊抄；细纲「复沓锚句」字段登记的原话（系统面板/誓词/案卷/固定专名）是唯一允许逐字一致的例外。主会话会用 `node scripts/check-outline-copy.js --check <正文文件...>` 复扫：连续重合 >15 字即视为誊抄证据，回正文改写。本 agent 默认工具不含 bash/Node，不得声称已运行脚本，只报告“已按规则自检，等待主会话复扫”。
